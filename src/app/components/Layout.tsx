    ...
    <div className="min-h-screen bg-white">
      ...
      <header className="fixed top-0 left-0 right-0 h-14 bg-white border-b border-gray-200 z-50">
        <div className="max-w-7xl mx-auto h-full flex">
          {/* 좌측: 프로필 영역 (사이드바 너비와 동일) */}
          <div className="w-72 flex-shrink-0 flex items-center gap-3 px-4 border-r border-gray-200">
            {/* 모바일 햄버거 메뉴 */}
            <button
            ...
            <img
              ...
              alt={user?.name ?? "익명"}
              className="w-8 h-8 rounded-full object-cover border border-gray-200 flex-shrink-0 hidden lg:block"
            />
            <span className="text-sm font-medium text-gray-800 truncate hidden lg:block">
              {user?.name ?? "익명"}
            </span>
          </div>
